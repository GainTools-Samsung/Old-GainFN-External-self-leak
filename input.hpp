...
namespace MouseController
{
	...
	static BOOLEAN WINAPI Init()
	{
		...
		LPVOID NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(skCrypt("win32u")), skCrypt("NtUserSendInput"));
		if (!NtUserSendInput_Addr)
		{
			NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(skCrypt("user32")), skCrypt("NtUserSendInput"));
			if (!NtUserSendInput_Addr)
			{
				// Windows 7 or lower detected
				NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(skCrypt("user32")), skCrypt("SendInput"));
				if (!NtUserSendInput_Addr)
					return FALSE;
			}
		}
		...
		// windows 8.1 / windows 10
		LPVOID NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(skCrypt("win32u")), skCrypt("NtUserGetAsyncKeyState"));
		if (!NtUserGetAsyncKeyState_Addr)
		{
			NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(skCrypt("user32")), skCrypt("NtUserGetAsyncKeyState"));
			if (!NtUserGetAsyncKeyState_Addr)
			{
				// Windows 7 or lower detected
				NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(skCrypt("user32")), skCrypt("GetAsyncKeyState"));
				if (!NtUserGetAsyncKeyState_Addr)
					return FALSE;
			}
		}
	...
	static BOOLEAN WINAPI Move_Mouse(int X, int Y)
	{
		...
		inp.mi.dx = X;
		inp.mi.dy = Y;
		inp.mi.dwFlags = TOUCHEVENTF_MOVE;
		return NtUserSendInput(1, &inp, sizeof(inp));
	}
